...
RUST_DIR="$PROJECT_DIR/rust-autopilot"

STATE_FILE="$RALPH_DIR/state.json"
LESSONS_FILE="$RALPH_DIR/journal/lessons.md"
GUIDE_FILE="$RALPH_DIR/GUIDE.md"
SEEDS_FILE="$RALPH_DIR/seeds.txt"
ROSTER_FILE="$RUST_DIR/src/bots/roster.rs"
HARNESS_FILE="$RALPH_DIR/harnesses/${HARNESS}.sh"
...
if [ ! -f "$HARNESS_FILE" ]; then
  ...
  ls "$RALPH_DIR/harnesses/"*.sh 2>/dev/null | xargs -I{} basename {} .sh | sed 's/^/  /'
  exit 1
fi
...
if [ ! -f "$ROSTER_FILE" ]; then
  log "ERROR: $ROSTER_FILE not found. Is rust-autopilot checked out?"
  exit 1
fi
...
for ITER in $(seq "$START_ITER" "$END_ITER"); do
  ...
  for LOOKBACK in $(seq $((ITER - 3 < 1 ? 1 : ITER - 3)) $((ITER - 1))); do
    ...
    REPORT="$RALPH_DIR/journal/iteration-${LB_PAD}.md"
    if [ -f "$REPORT" ]; then
...
")
  ...
  PREV_RUNS_CSV="$RALPH_DIR/scores/iteration-${PREV_PAD}/runs.csv"
...
## YOUR TASK
Edit the \`evolve-candidate\` SearchConfig in \`rust-autopilot/src/bots/roster.rs\`.
Change ONE parameter (or a small related group). Explain your reasoning as a comment next to the change.
Read \`ralph-autopilot/GUIDE.md\` for parameter ranges, interaction rules, and strategy tips.

IMPORTANT: The working directory is the project root. Use \`rust-autopilot/\` prefix for Rust source paths and \`ralph-autopilot/\` prefix for evolution state/artifacts.
EDIT_PROMPT
  else
...
You are an AI bot designer iteratively improving an Asteroids autopilot bot. This is iteration **$ITER** of an automated progressive learning loop. You are running headlessly inside \`$HARNESS_DISPLAY\` in the project root directory. The Rust code is in \`rust-autopilot/\` and evolution state/artifacts are in \`ralph-autopilot/\`.
...
Read these files:
- \`ralph-autopilot/GUIDE.md\` — parameter ranges, interaction rules, reference scores, strategy
- \`ralph-autopilot/journal/lessons.md\` — cumulative learnings from all prior iterations
- \`rust-autopilot/src/bots/roster.rs\` — find the \`evolve-candidate\` SearchConfig at the bottom of \`search_bot_configs()\`

$(if [ "$PREV_ITER" -gt 0 ]; then echo "Since this is not the first iteration, also read:
- \`ralph-autopilot/scores/iteration-${PREV_PAD}/summary.json\` — last benchmark results
- \`ralph-autopilot/scores/iteration-${PREV_PAD}/runs.csv\` — per-seed breakdown
- \`ralph-autopilot/journal/iteration-${PREV_PAD}.md\` — last iteration's report"; fi)
...
\`\`\`bash
cd rust-autopilot && cargo run --release -- codex-intel-run --bot evolve-candidate --seed <WORST_SEED_HEX> --max-frames 108000
\`\`\`
...
\`\`\`bash
cd rust-autopilot && cargo build --release 2>&1
\`\`\`
If build fails, fix and retry.

\`\`\`bash
cd rust-autopilot && cargo run --release -- benchmark \\
  ...
  --save-top 3 \\
  --jobs 8 \\
  --out-dir ralph-autopilot/scores/iteration-${ITER_PAD}
\`\`\`

### Step 5: Evaluate Results
Read \`ralph-autopilot/scores/iteration-${ITER_PAD}/summary.json\` and \`runs.csv\`.
...
Write to \`ralph-autopilot/journal/iteration-${ITER_PAD}.md\` — this MUST include:
...
### Step 7: Update State
Read \`ralph-autopilot/state.json\`, then write it back with these changes:
...
- If **IMPROVED** (avg_score > best_avg_score):
  ...
  - Snapshot config: copy the evolve-candidate SearchConfig block to \`ralph-autopilot/designs/iteration-${ITER_PAD}.rs\`
- If **REGRESSED** (avg_score < best_avg_score by more than 2%):
  - Increment \`consecutive_regressions\` and \`total_regressions\`
  - **REVERT**: read \`ralph-autopilot/designs/iteration-$(printf '%03d' $(read_state best_iteration)).rs\` and restore that config into roster.rs
  - Then run \`cd rust-autopilot && cargo build --release\` to verify the revert compiles
...
**ALWAYS** append to \`ralph-autopilot/journal/lessons.md\` under "## Iteration History". Be SPECIFIC:
...
## LESSONS LEARNED (read ralph-autopilot/journal/lessons.md for full version)
...
AGENT_PROMPT
  ...
  OUTPUT_LOG="$RALPH_DIR/journal/${HARNESS}-output-${ITER_PAD}.log"
  SCORES_DIR="$RALPH_DIR/scores/iteration-${ITER_PAD}"

  log "Launching $HARNESS_DISPLAY for iteration $ITER..."
  HARNESS_START=$(date +%s)

  set +e
  harness_exec "$PROMPT_FILE" "$OUTPUT_LOG" "$PROJECT_DIR"
  HARNESS_EXIT=$?
  set -e
  ...
  if [ "${HARNESS_NEEDS_EXTERNAL_BENCH:-false}" = "true" ]; then
    ...
    if [ "$BUILD_EXIT" -ne 0 ]; then
      ...
      BEST_DESIGN="$RALPH_DIR/designs/iteration-${BEST_ITER_PAD}.rs"
...
"
    ...
    cargo run --release -- benchmark \
      ...
      --jobs 8 \
      --out-dir "$SCORES_DIR" \
      2>&1 | tee "$RALPH_DIR/journal/bench-output-${ITER_PAD}.log"
...
{verdict}
'''
with open(f'{ralph_dir}/journal/iteration-{iter_pad}.md', 'w') as f:
    f.write(report)
...
m = re.search(r'// ── evolve-candidate.*?},', text, re.DOTALL)
if m:
    with open('$RALPH_DIR/designs/iteration-${ITER_PAD}.rs', 'w') as f:
