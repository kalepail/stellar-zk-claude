...
def ensure_binary(repo_root: Path) -> Path:
    bin_path = repo_root / "rust-autopilot" / "target" / "release" / "rust-autopilot"
    if bin_path.exists():
        return bin_path
    ...
    run_cmd(
        [
            ...
            "--release",
            "--manifest-path",
            str(repo_root / "rust-autopilot" / "Cargo.toml"),
        ],
        cwd=repo_root,
    )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--seeds-file",
        default="codex-autopilot/seeds/screen-seeds.txt",
    )
...
def main() -> int:
    ...
    lab_root = repo_root / "codex-autopilot"
    seeds_file = (repo_root / args.seeds_file).resolve()
    base_profile_path = lab_root / "profiles" / "base.json"
    champion_profile_path = lab_root / "profiles" / "champion.json"
    active_profile_path = (
        repo_root / "rust-autopilot" / "codex-" / "state" / "adaptive-profile.json"
    )
    ...
    if not active_profile_path.exists():
        raise FileNotFoundError(f"active profile not found: {active_profile_path}")
