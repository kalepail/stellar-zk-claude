...
Fresh autopilot lab focused on one goal: iteratively tune a high-scoring Codex ship without the sprawl in `rust-autopilot`.
...
```bash
./codex-autopilot/scripts/run-super-score-loop.sh
```
...
```bash
./codex-autopilot/scripts/iterative-search.py \
  --iterations 8 \
  --candidates 8 \
  --max-frames 108000 \
