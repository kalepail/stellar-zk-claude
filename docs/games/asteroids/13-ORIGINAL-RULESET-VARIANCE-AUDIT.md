...
- Saucer bullet hard cap is now `2` (TS + zk).
- Saucer bullets and ship bullets now use `72`-frame lifetime (TS + zk).
- Saucers are side-entry with immediate X-bound cull (no offscreen X grace margin).
- Saucer spawn/fire pauses while ship is not visible.
- Saucer-asteroid collisions are active in both engines.
- Small saucer score is `990` in both engines.
- Ship fire gating now uses edge-triggered latch + cooldown semantics (TS + zk).
- Saucer firing cadence now uses deterministic pressure-based cooldown ranges (TS + zk).

## Update (2026-02-11)
Current code state corrections:
- Max concurrent saucers are `1` (waves `<4`), `2` (waves `4..6`), `3` (waves `>=7`) in both engines.
- Small-saucer aiming error scales with pressure (`wave + anti-lurk`), not direct score bands.

Still intentionally divergent from original:
- No hyperspace.
- Modernized wave/pressure model (including scaling saucer concurrency beyond 1).
- Wrapped shortest-distance collision model remains global.
...
| V-07 | Saucer bullet slot cap | Saucer bullets are hard-capped at `2` (TS + zk). | Original allocates 2 saucer bullet slots. | Resolved for slot count. | Low | High | Keep current cap unless intentionally diverging again. |
| V-08 | Ship bullet autofire behavior | Edge-triggered anti-autofire latch + cooldown gate (`shipFireLatch` / `ship_fire_latch`) in TS + zk. | Original ship-fire logic is edge-triggered with slot/timer constraints. | Closely aligned on hold/release semantics; not a known material divergence now. | Low | Medium | Keep current gate; revisit only if pursuing byte-level ROM cadence emulation. |
| V-09 | Ship bullet lifetime | 72 frames (`src/game/constants.ts`, Rust constants mirror). | Original ship shots initialize timer `#$12` and decrement every 4th frame (effective ~72-frame lifespan at original frame cadence). | Effective lifetime now aligns with arcade semantics; cadence implementation remains simplified. | Low | Medium | Keep 72-frame lifetime; optionally port exact 4-frame decrement semantics. |
| V-10 | Saucer bullet lifetime | 72 frames with 2-slot cap (`src/game/constants.ts`, Rust constants mirror). | Original saucer shots share the same timer cadence model and 2-slot limit. | Effective lifetime and slot pressure are now close to original. | Low | Medium | Optionally port exact cadence semantics only if needed. |
...
| V-12 | Small saucer accuracy curve | Error shrinks with pressure (`wave + anti-lurk`) in TS + zk. | Original uses mask-table randomization and higher-score accuracy inflection (notably around 35,000). | Accuracy progression curve is fundamentally different. | High | High | Port original randomization/mask-table style aiming in classic mode. |
| V-13 | Saucer firing cadence | Deterministic pressure-based cooldown ranges (sampled via game RNG), not fixed reload, in TS + zk. | Original uses timer-based cadence with fixed re-arm values in saucer-shoot path. | Cadence model remains intentionally modernized. | Medium | Medium | Tune cooldown ranges or add classic mode if closer parity is required. |
| V-14 | Small saucer scoring | `SCORE_SMALL_SAUCER = 990` (`src/game/constants.ts`, Rust constants mirror). | Original score-add path uses BCD `#$99` (990 in tens-based internal representation). | Resolved. | Low | High | Keep 990 unless intentionally retuning score economy. |
...
| V-19 | Asteroid-cap enforcement edge case | Split logic now enforces strict cap (`alive <= ASTEROID_CAP`) in TS + zk. | Original uses fixed slot model for asteroid objects. | Resolved for hard-cap enforcement. | Low | High | Keep current strict invariant. |
...
| V-21 | Spawn location policy | Center-first, then deterministic best-open-area respawn with edge padding. | Original uses center/visibility timing plus hyperspace-specific logic. | Spatial risk profile still intentionally modernized. | Medium | High | Keep unless strict cabinet behavior is required. |
| V-22 | Saucer vs asteroid interaction | Saucer-asteroid collision destroys saucer in TS + zk. | Original saucers can be destroyed by asteroid collisions. | Resolved. | Low | High | Keep current behavior. |
