...
- Ship fire gating now uses edge-style shift-register semantics (TS + zk).
- Saucer firing cadence now uses fixed deterministic reload (`10` frames, TS + zk).
...
| V-06 | Concurrent saucers | Max saucers by wave: `1`, `2`, then scales `3..6` in late waves (`src/game/AsteroidsGame.ts`, `risc0-asteroids-verifier/asteroids-core/src/sim/mod.rs`). | Original runtime model is single active saucer status slot. | Multi-saucer concurrency is non-original and materially harder. | High | High | Cap to 1 in classic mode. |
...
| V-08 | Ship bullet autofire behavior | Edge-triggered anti-autofire shift-register gate (`ship_fire_shift_reg` in TS + zk). | Original ship-fire logic is edge-triggered with slot/timer constraints. | Closely aligned on hold/release semantics; not a known material divergence now. | Low | Medium | Keep current gate; revisit only if pursuing byte-level ROM cadence emulation. |
...
| V-13 | Saucer firing cadence | Fixed deterministic reload (`SAUCER_FIRE_RELOAD_FRAMES = 10`) in TS + zk. | Original uses timer-based cadence with fixed re-arm values in saucer-shoot path. | Core cadence model now aligned; remaining difficulty differences come from other systems (concurrency/pressure). | Low | Medium | Keep fixed reload; tune only with explicit design intent. |
