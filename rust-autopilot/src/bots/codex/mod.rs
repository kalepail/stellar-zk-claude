...
use asteroids_verifier_core::sim::{
    AsteroidSizeSnapshot, BulletSnapshot, WorldSnapshot,
};
...
pub(super) fn load_adaptive_profile() -> AdaptiveProfile {
    let Ok(raw) = fs::read_to_string(ADAPTIVE_PROFILE_PATH) else {
...
fn apply_adaptive_profile(base: PotentialConfig, profile: AdaptiveProfile) -> PotentialConfig {
    PotentialConfig {
        ...
        min_fire_quality: (base.min_fire_quality + profile.min_fire_quality_delta).clamp(0.05, 0.65),
    ...
    fn evaluate_action(&self, world: &WorldSnapshot, action: u8) -> f64 {
        ...
        if input.fire {
            if can_fire(world, pred) {
                ...
                if discipline_ok && !duplicate && fire_quality >= min_quality {
                    fire_term += weights.fire_reward * (0.32 + 0.68 * fire_alignment) * fire_quality;
        ...
        if matches!(self.mode, StanceMode::Panic) && nearest_threat < self.cfg.panic_distance_px * 1.2 {
...
impl RolloutBot {
    ...
    fn evaluate_rollout_action(
        ...
        let input = decode_input_byte(action);
        let pred = after.ship;
        let target = best_target_at_step(world, pred, step_index, self.cfg.lookahead_frames + 40.0, 1.0);
    ...
    fn rollout_value(
        ...
        for action in ACTIONS_ROLLOUT {
            ...
            let immediate = self.evaluate_rollout_action(world, state, &next, action, fired, step_index);
...
fn entity_risk(
    ...
    let approach = torus_relative_approach(pred.x, pred.y, pred.vx, pred.vy, ex, ey, evx, evy, horizon);
    ...
    let time_boost = 1.0 + ((horizon - approach.t_closest) / horizon.max(1.0)).clamp(0.0, 1.0) * 0.35;
...
fn best_target_internal(
    ...
    let mut consider = |target: MovingTarget| {
        ...
        value += angle_bonus;
        value *= 1.0 + (1.0 - (intercept.intercept_frames / lead_cap.max(1.0))).clamp(0.0, 1.0) * 0.18;
...
fn estimate_fire_quality_at_step(world: &WorldSnapshot, ship: PredictedShip, step_index: u8) -> f64 {
    ...
    for asteroid in &world.asteroids {
        let (x, y) = advance_entity(asteroid.x, asteroid.y, asteroid.vx, asteroid.vy, step_index);
        consider(x, y, asteroid.vx, asteroid.vy, asteroid.radius, asteroid_weight(asteroid.size));
    }
...
fn nearest_threat_distance_px_at_step(world: &WorldSnapshot, ship: PredictedShip, step_index: u8) -> f64 {
...
fn nearest_saucer_distance_px_at_step(world: &WorldSnapshot, ship: PredictedShip, step_index: u8) -> f64 {
    let mut nearest = f64::MAX;
    for saucer in &world.saucers {
        let (x, y) = advance_entity(saucer.x, saucer.y, saucer.vx, saucer.vy, step_index);
        let (dx, dy) = torus_delta(ship.x, ship.y, x, y);
...
fn target_already_covered_by_ship_bullets(target: MovingTarget, bullets: &[BulletSnapshot]) -> bool {
    bullets
        .iter()
...
pub(super) fn describe_codex_bots() -> Vec<(&'static str, &'static str)> {
    ...
    out.extend(stance_bot_configs().iter().map(|cfg| (cfg.id, cfg.description)));
    out.extend(rollout_bot_configs().iter().map(|cfg| (cfg.id, cfg.description)));
    out
}
...
pub(super) fn codex_manifest_entries() -> Vec<(&'static str, &'static str, serde_json::Value)> {
    ...
    let mut adaptive_cfg =
        serde_json::to_value(apply_adaptive_profile(adaptive_base_potential_config(), adaptive_profile))
            .expect("adaptive potential config should serialize");
    adaptive_cfg["adaptive_profile_path"] =
        serde_json::Value::String(ADAPTIVE_PROFILE_PATH.to_string());
