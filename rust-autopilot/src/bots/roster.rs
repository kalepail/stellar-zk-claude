...
pub(super) fn record_locked_bot_configs() -> &'static [ReplayConfig] {
    &[ReplayConfig {
        ...
        tape_path: RECORD_ENDURANCEX_TAPE_PATH,
...
fn tape_fingerprint(path: &str) -> Option<String> {
    let bytes = fs::read(path).ok()?;
    let digest = crc32(&bytes);
    Some(format!("crc32:{digest:08x}:len:{}", bytes.len()))
}
...
pub fn bot_manifest_entries() -> Vec<BotManifestEntry> {
    ...
    for cfg in record_locked_bot_configs() {
        let mut config =
            serde_json::to_value(cfg).expect("record-lock bot config should serialize");
        if let Some(tape_digest) = tape_fingerprint(cfg.tape_path) {
            config["tape_fingerprint"] = serde_json::Value::String(tape_digest);
        }
        out.push(BotManifestEntry {
            id: cfg.id.to_string(),
