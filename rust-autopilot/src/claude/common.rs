...
impl PredictedShip {
    pub fn from_world(world: &WorldSnapshot) -> Self {
        Self {
            x: world.ship.x,
            y: world.ship.y,
            vx: world.ship.vx,
...
pub fn estimate_fire_quality(pred: PredictedShip, world: &WorldSnapshot) -> f64 {
    ...
    let mut consider = |x: i32, y: i32, vx: i32, vy: i32, radius: i32, weight: f64| {
        ...
        let (closest, t) =
            projectile_wrap_closest_approach(start_x, start_y, bullet_vx, bullet_vy, x, y, vx, vy, max_t);
    ...
    for asteroid in &world.asteroids {
        ...
        let w = asteroid_target_weight(asteroid.size);
        consider(
            asteroid.x, asteroid.y, asteroid.vx, asteroid.vy, asteroid.radius, w,
        );
    }
...
pub fn asteroid_score(size: AsteroidSizeSnapshot) -> u32 {
    match size {
        AsteroidSizeSnapshot::Large => 20,
        AsteroidSizeSnapshot::Medium => 50,
        AsteroidSizeSnapshot::Small => 100,
    }
}
...
pub fn best_target(world: &WorldSnapshot, pred: PredictedShip) -> Option<TargetInfo> {
    ...
    let mut consider =
        |x: i32, y: i32, vx: i32, vy: i32, radius: i32, weight: f64| {
            if weight <= 0.0 {
                return;
            }
...
pub fn bullet_tracks_target(bullet: &BulletSnapshot, tx: i32, ty: i32, tvx: i32, tvy: i32, tradius: i32) -> bool {
...
pub fn target_already_covered(bullets: &[BulletSnapshot], tx: i32, ty: i32, tvx: i32, tvy: i32, tradius: i32) -> bool {
    bullets.iter().any(|b| bullet_tracks_target(b, tx, ty, tvx, tvy, tradius))
}
...
pub fn danger_at_point(
    ...
    for asteroid in &world.asteroids {
        ...
        let approach = torus_relative_approach(
            x, y, vx, vy, asteroid.x, asteroid.y, asteroid.vx, asteroid.vy, horizon,
        );
        let safe = (14 + asteroid.radius + 8) as f64;
        let closeness = (safe / (approach.closest_px + 1.0)).powf(2.0);
        let closing = if approach.dot < 0.0 { 1.25 } else { 0.9 };
        danger += asteroid_weight * closeness * closing;
    }
    ...
    for saucer in &world.saucers {
        ...
        let approach = torus_relative_approach(
            x, y, vx, vy, saucer.x, saucer.y, saucer.vx, saucer.vy, horizon,
        );
        let safe = (14 + saucer.radius + 8) as f64;
        let closeness = (safe / (approach.closest_px + 1.0)).powf(2.0);
    ...
    for bullet in &world.saucer_bullets {
        ...
        let approach = torus_relative_approach(
            x, y, vx, vy, bullet.x, bullet.y, bullet.vx, bullet.vy, horizon,
        );
        let safe = (14 + bullet.radius + 8) as f64;
        let closeness = (safe / (approach.closest_px + 1.0)).powf(2.2);
        let closing = if approach.dot < 0.0 { 1.4 } else { 0.9 };
        danger += bullet_weight * closeness * closing;
    }

    danger
}

// ── Fire discipline gate (shared) ───────────────────────────────────
