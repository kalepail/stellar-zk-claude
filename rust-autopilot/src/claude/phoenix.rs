...
impl PhoenixBot {
    ...
    fn entity_risk(
        ...
        let time_boost =
            1.0 + ((lookahead - approach.t_closest) / lookahead) * 0.45;
    ...
    fn action_utility(&self, world: &WorldSnapshot, input_byte: u8) -> f64 {
        ...
        for asteroid in &world.asteroids {
            ...
            risk += self.entity_risk(
                ...
                asteroid.radius, params.risk_weight_asteroid, params.lookahead,
            );
        }
        for saucer in &world.saucers {
            ...
            risk += self.entity_risk(
                ...
                saucer.radius, w, params.lookahead,
            );
        }
        for bullet in &world.saucer_bullets {
            ...
            risk += self.entity_risk(
                ...
                bullet.radius, params.risk_weight_bullet, params.lookahead,
            );
        }
            ...
            let is_duplicate = target_plan
                .map(|p| {
                    target_already_covered(
                        &world.bullets,
                        p.target_x, p.target_y, p.target_vx, p.target_vy, p.target_radius,
                    )
                })
                .unwrap_or(false);
            let discipline_ok = disciplined_fire_ok(
                active, shortest, fire_quality, params.min_fire_quality,
                nearest_saucer, nearest_threat, is_duplicate,
            );
            let emergency_saucer = nearest_saucer < 95.0 && fire_quality + 0.08 >= params.min_fire_quality;

            if !is_duplicate && discipline_ok && (fire_quality >= params.min_fire_quality || emergency_saucer) {
                ...
                fire_term -= params.miss_fire_penalty * (params.min_fire_quality - fire_quality).max(0.0) * 0.45;
