...
pub fn evolve_from_intel(
    ...
    let avg_nearest_threat = if death_count_for_avg == 0 { 0.0 } else { sum_nearest_threat / death_count_for_avg as f64 };
    let avg_min_edge = if death_count_for_avg == 0 { 0.0 } else { sum_min_edge / death_count_for_avg as f64 };
    let avg_wave_at_death = if death_count_for_avg == 0 { 0.0 } else { sum_wave_at_death / death_count_for_avg as f64 };
    ...
    let saucer_death_ratio = if total_deaths == 0 { 0.0 } else { saucer_deaths as f64 / total_deaths as f64 };
    let asteroid_death_ratio = if total_deaths == 0 { 0.0 } else { asteroid_deaths as f64 / total_deaths as f64 };
    ...
    if avg_nearest_threat < 50.0 {
        death_insights.push(format!("Deaths at very close range ({:.0}px avg): need faster evasion response.", avg_nearest_threat));
    }
    if avg_wave_at_death > 6.0 {
        death_insights.push(format!("Deaths concentrated in late waves ({:.1} avg): increase late-game survival weight.", avg_wave_at_death));
    }
    if deaths_per_10k > 5.0 {
        death_insights.push(format!("High death rate ({:.1}/10k frames): global risk sensitivity increase needed.", deaths_per_10k));
    }
    ...
    if miss_rate > 0.55 {
        shot_insights.push(format!("Very high miss rate ({:.0}%): increase min fire quality and shot penalty.", miss_rate * 100.0));
    }
    if miss_rate > 0.45 {
        shot_insights.push(format!("High miss rate ({:.0}%): increase fire quality floor.", miss_rate * 100.0));
    }
    if avg_frames_between_shots < 15.0 {
        shot_insights.push(format!("Shooting too frequently ({:.1} frames between shots): increase shot discipline.", avg_frames_between_shots));
    }
    if hit_rate > 0.55 {
        shot_insights.push(format!("Good hit rate ({:.0}%): can afford to be slightly more aggressive.", hit_rate * 100.0));
    }
...
impl EvolvedBot {
    ...
    fn entity_risk(
        &self,
        pred: PredictedShip,
        ex: i32, ey: i32, evx: i32, evy: i32,
        radius: i32, weight: f64,
    ) -> f64 {
        let approach = torus_relative_approach(
            pred.x, pred.y, pred.vx, pred.vy,
            ex, ey, evx, evy, self.cfg.lookahead,
        );
        ...
        let time_boost = 1.0 + ((self.cfg.lookahead - approach.t_closest) / self.cfg.lookahead) * 0.45;
    ...
    fn action_utility(&self, world: &WorldSnapshot, action: u8) -> f64 {
        ...
        for asteroid in &world.asteroids {
            ...
            risk += self.entity_risk(
                pred, asteroid.x, asteroid.y, asteroid.vx, asteroid.vy,
                asteroid.radius, self.cfg.risk_weight_asteroid,
            );
        }
        for saucer in &world.saucers {
            if !saucer.alive { continue; }
            let w = if saucer.small { self.cfg.risk_weight_saucer * 1.28 } else { self.cfg.risk_weight_saucer };
            risk += self.entity_risk(
                pred, saucer.x, saucer.y, saucer.vx, saucer.vy,
                saucer.radius, w,
            );
        }
        for bullet in &world.saucer_bullets {
            ...
            risk += self.entity_risk(
                pred, bullet.x, bullet.y, bullet.vx, bullet.vy,
                bullet.radius, self.cfg.risk_weight_bullet,
            );
        }
        ...
        if world.next_extra_life_score > world.score {
            ...
            if to_next <= 1_500 { aggression *= 1.12; }
            if to_next <= 500 { aggression *= 1.2; }
        }
        ...
        if let Some(plan) = target_plan {
            ...
            if plan.distance_px < 280.0 { attack += 0.16; }
            if plan.intercept_frames <= 14.0 { attack += 0.05; }
        }

        // Position
        use asteroids_verifier_core::fixed_point::shortest_delta_q12_4;
        let cx = shortest_delta_q12_4(pred.x, WORLD_WIDTH_Q12_4 / 2, WORLD_WIDTH_Q12_4) as f64 / 16.0;
        let cy = shortest_delta_q12_4(pred.y, WORLD_HEIGHT_Q12_4 / 2, WORLD_HEIGHT_Q12_4) as f64 / 16.0;
        ...
        if input.fire && world.bullets.len() < SHIP_BULLET_LIMIT && pred.fire_cooldown <= 0 {
            ...
            let is_duplicate = target_plan
                .map(|p| target_already_covered(
                    &world.bullets, p.target_x, p.target_y, p.target_vx, p.target_vy, p.target_radius,
                ))
                .unwrap_or(false);
            let discipline_ok = disciplined_fire_ok(
                active, shortest, fire_quality, self.cfg.min_fire_quality,
                nearest_saucer, nearest_threat, is_duplicate,
            );
            let emergency = nearest_saucer < 95.0 && fire_quality + 0.08 >= self.cfg.min_fire_quality;

            if !is_duplicate && discipline_ok && (fire_quality >= self.cfg.min_fire_quality || emergency) {
                ...
                fire_term -= self.cfg.miss_fire_penalty * (self.cfg.min_fire_quality - fire_quality).max(0.0) * 0.45;
        ...
        if input.left || input.right {
            ...
            if nearest_threat > 180.0 { control_term -= self.cfg.turn_penalty * 0.45; }
        }
        if input.thrust {
...
impl AutopilotBot for EvolvedBot {
    ...
    fn next_input(&mut self, world: &WorldSnapshot) -> FrameInput {
        if world.is_game_over || !world.ship.can_control {
            return FrameInput { left: false, right: false, thrust: false, fire: false };
        }
