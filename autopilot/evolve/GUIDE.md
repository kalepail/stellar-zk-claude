# Autopilot — Progressive Evolution Loop
...
These ranges are based on cross-analysis of:

- `codex-tuner/` (adaptive profile tuning)
- `archive/claude-autopilot/` (standalone action-search lab)

The ranges are deliberately wide to allow exploration.
...
3. **Position Pair** (change together):
   ...
   - `codex-tuner/` uses ~2x edge penalty as its top tuning win.
...
| codex-tuner champion | 67K | 226K | extreme edge penalty, conservative fire, high mobility |
