...
impl NavigatorBot {
    ...
    fn evaluate_action(&self, world: &WorldSnapshot, action: u8) -> f64 {
        ...
        let danger = danger_at_point(
            world, pred.x, pred.y, pred.vx, pred.vy, 20.0, // horizon
        ...
        let future_danger = danger_at_point(
            world, future_x, future_y, pred.vx, pred.vy, 16.0, 1.2, 1.8, 2.4,
        );
        ...
        if let Some(target) = best_target(world, pred) {
            ...
            if input.fire && world.bullets.len() < SHIP_BULLET_LIMIT && pred.fire_cooldown <= 0 {
                ...
                {
                    fire_term += 1.1
                        * fire_alignment_score(pred, target.aim_angle)
                        * (0.35 + 0.65 * fire_quality);
                    fire_term -= 0.75 * 0.72;
                    if lurk_active {
                        fire_term += 0.3;
                    }
        ...
        let control_scale = if danger > 2.0 {
            0.2
        } else if danger > 1.0 {
            0.4
        } else {
